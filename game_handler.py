...
class GameHandler:
    ...
    async def process(self, action: Optional[str] = None) -> None:
        ...
        if step in self.handlers:
            print(step, action)
            await self.handlers[step](action)
        else:
            await self.io.output(f"Unknown step: {step}")

    def change_user_step(self, next_step: str) -> None:
        self.users_data.update_step(self.username, next_step)

    def add_user_to_room(self, code: int) -> None:
        self.users_data.update_code(self.username, code)

    def get_user_room(self) -> GameStateRoom:
        code = self.users_data.get_user_info(self.username).code
        return GameStateRoom(code)

    async def handle_unknown_user(self, action: Optional[str]) -> None:
        await self.io.output(master_message("app_start"))
        await self.io.output("Действия: [Создать игру] или введите код игры")
        self.change_user_step("start")

    async def handle_start(self, action: Optional[str]) -> None:
        if action == "Создать игру":
            code = random.randint(10000, 99999)
            room = GameStateRoom(code)
            room.create(self.username)
            self.add_user_to_room(code)
            await self.io.output(master_message("input_num_players"))
            ...
            self.add_user_to_room(code)
            await self.io.output("Вы присоединились к игре")
            await self.handle_waiting_players()
            self.change_user_step("waiting_players")

    async def handle_exit(self, action: Optional[str]) -> None:
        self.users_data.delete(self.username)

    async def handle_input_num_players(self, action: Optional[str]) -> None:
        try:
            ...
            if 1 <= num_players <= 8:
                ...
                await self.io.output(master_message("input_setting"))
            else:
                await self.io.output(master_message("wrong_num_players"))
        except ValueError:
            await self.io.output(master_message("wrong_num_players"))

    async def handle_input_setting(self, action: Optional[str]) -> None:
        if len(action) < 42:
            room = self.get_user_room()
            room.add_setting(action)
            num_players = room.get_state().num_players
            task_executor.add_game_start_task(action, num_players)
            await self.io.output(f"Сообщите своим друзьям код, чтобы они могли присоединиться к игре: {room.code}")
            self.change_user_step("waiting_players")
        else:
            await self.io.output(master_message("wrong_setting"))

    async def handle_waiting_players(self, action: Optional[str] = None) -> None:
        room = self.get_user_room()
        ...
        if game_state and game_start_generator.exists():
            ...
            await self.io.output(f"Сценарий: {game_state.setting}")
            await self.io.output(master_message("game_start", params=asdict(game_start.crime_info)))
            ...
            for character in game_start.characters:
                ...
                await self.io.output(char_info)
            self.change_user_step("character_choice")
            await self.io.output("Введите имя персонажа:")
        else:
            status = f"Ожидаем игроков ({len(game_state.users)}/{game_state.num_players})"
            await self.io.output(status)
            await self.io.output("Действия: [Обновить]")

    async def handle_character_choice(self, action: Optional[str]) -> None:
        room = self.get_user_room()
        user_character = action

        if user_character:
            room.add_user(self.username, action)
            await self.io.output(f"Вы выбрали: {user_character}")
            await self.handle_waiting_start()
            self.change_user_step("waiting_start")

    async def handle_waiting_start(self, action: Optional[str] = None) -> None:
        room = self.get_user_room()
        game_state = room.get_state()
    ...
    async def handle_player_memories(self, action: Optional[str]) -> None:
        room = self.get_user_room()
        game_state = room.get_state()
        ...
        await self.io.output(master_message(message_type, params=asdict(memories)))
        self.change_user_step("discovery_action")

    async def handle_discovery_action(self, action: Optional[str]) -> None:
        if action is None:
            await self.io.output("Опишите ваше действие:")
            return
        ...
        await self.io.output("Действие сохранено")

    async def handle_waiting_next_round(self, action: Optional[str]) -> None:
        if action == "Готово":
            room = self.get_user_room()
            game_state = room.get_state()
            game_start = GameStartGenerator(game_state.setting, game_state.num_players).generate()
            ...
            await self.io.output(next_round)
            await self.io.output(
                "Обсудите в последний раз новую информацию и затем проголосуйте за персонажа, "
                "которого больше всего подозреваете. "
                "Тот кто наберет больше всех голосов, должен раскрыть свой секрет."
            )
        else:
            await self.io.output("Рекомендуемое время обсуждения: 10 мин")
            await self.io.output("Действия: [Готово]")